-- Migration PostgreSQL: Gestion dynamique du contenu de présentation
-- Date: 2025-12-13
...
CREATE TRIGGER update_contenu_presentation_updated_at 
    BEFORE UPDATE ON contenu_presentation 
    FOR EACH ROW 
...
-- Trigger pour updated_at
CREATE TRIGGER update_valeurs_updated_at 
    BEFORE UPDATE ON valeurs 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();
...
-- Index
CREATE INDEX IF NOT EXISTS idx_contenu_presentation_actif ON contenu_presentation(actif);
CREATE INDEX IF NOT EXISTS idx_contenu_presentation_ordre ON contenu_presentation(ordre);
CREATE INDEX IF NOT EXISTS idx_valeurs_actif ON valeurs(actif);
CREATE INDEX IF NOT EXISTS idx_valeurs_ordre ON valeurs(ordre);
